...
async def http_start_job_search(req: func.HttpRequest, client: df.DurableOrchestrationClient):
...
def job_search_orchestrator(context: df.DurableOrchestrationContext):
    ...
    # Search for jobs
    raw_search_results: LinkedinSearchLoad = yield context.call_activity("search", SearchParam(keywords, diff))

    # Store in cosmos
    yield context.call_activity("persists_query_in_cosmos", raw_search_results)

    # Run processing for each job result
    tasks = [context.call_sub_orchestrator("job_info_orchestrator", result) for result in raw_search_results.jobs_reply]
...
def search(query: SearchParam):
    ...
    jobs_reply = api.search_remote_jobs(query.keywords, listed_offset_seconds=listed_offset_seconds)
    ...
    storage.persist_on_storage("raw/job_searches/" + search_load.get_path_part(), search_load.get_file_name(), search_load)

    return search_load

@app.activity_trigger(input_name="input")
@app.cosmos_db_output(arg_name="cosmos", database_name="linkedinjobs", container_name="query", partition_key="/query", create_if_not_exists=True, connection="CosmosConnection")
def persists_query_in_cosmos(input: LinkedinSearchLoad, cosmos: func.Out[func.DocumentList]):
...
@app.activity_trigger(input_name="job")
@app.cosmos_db_output(arg_name="cosmos", database_name="linkedinjobs", container_name="jobs", connection="CosmosConnection")
